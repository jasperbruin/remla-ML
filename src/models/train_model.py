...
if __name__ == "__main__":
    x_train = load_pickle(os.path.join(PARAMS["tokenized_folder"],
                                       "x_train.pickle"))
    y_train = load_pickle(os.path.join(PARAMS["tokenized_folder"],
                                       "y_train.pickle"))
    x_val = load_pickle(os.path.join(PARAMS["tokenized_folder"],
                                     "x_val.pickle"))
    y_val = load_pickle(os.path.join(PARAMS["tokenized_folder"],
                                     "y_val.pickle"))

    if not os.path.exists(PARAMS["trained_folder"]):
        os.makedirs(PARAMS["trained_folder"])

    char_index = load_pickle(os.path.join(PARAMS["tokenized_folder"],
                                          "char_index.pickle"))
    training_model = create_model(len(char_index),
